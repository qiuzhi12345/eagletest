...
S8 ate_RX_GAIN_CHECK_LOW_hdb[14]  = {-8, -1, -3, -3, -1, -2, -6, -2, -2, -2, -2, -2, -2, -2};
S8 ate_RX_GAIN_CHECK_HIGH_hdb[14] = { 0,  7,  2,  2,  4,  3, -2,  2,  2,  2,  2,  2,  2,  2};
...
S8 ate_RX_SWITCH_GAIN_HIGH_db[3] = { 8,   2, 5};
S8 ate_RX_SWITCH_GAIN_LOW_db[3] =  { 3,  -2, 2};
...
#define FB_RX_NUM_SUM_LOW 80

#define DUT_RX_LOST_HIGH 2
#define DUT_RX_LOST_LOW 0

#define FB_RXRSSI_HIGH 54
#define FB_RXRSSI_LOW 46

#define DUT_RXRSSI_HIGH 62
#define DUT_RXRSSI_LOW 54
...
//#endif
    ...
    check_result_t = (check_result[3] << 8*3) | (check_result[2] << 8*2) | 
		...
		for(i=0;i<11;i++){
            if(i<6){chip_id_byte[i]=rx_gain_check_tot_pwr[9+i];}
            else{chip_id_byte[i]=rx_gain_check_sig_pwr[10+i-6];}
            
    }
    for(i=11;i<16;i++){
            if(i>14){chip_id_byte[i]=*CHIP_VERSION;}
            else{chip_id_byte[i]=CHIP_ID[i-11];}
            
    }

    if (ate_check_print) PHY_PRINT("\nCHIP_VERSION: 0x%x\n", *CHIP_VERSION);
    for(i=3;i>=0;i--){
    		CHIP_ID_t[i]=(chip_id_byte[i*4+3] << 8*3) | (chip_id_byte[i*4+2] << 8*2) | (chip_id_byte[i*4+1] << 8*1) | (chip_id_byte[i*4+0] << 8*0);
				if (ate_check_print) PHY_PRINT("CHIP_ID_%d: 0x%x\n", i,CHIP_ID_t[i]);
    	}    
    ...
    U8 CHIP_VERSION_t=*CHIP_VERSION;
		check_ate_data( (int)CHIP_VERSION_t, 0, 1, CHIP_ID_CRC_ITEM);	//chip version check
    check_ate_data(CHIP_ID_t==0, 0, 0, CHIP_ID_CRC_ITEM); //chip ID ==0 check
...
//   cscExec->Datalog()->printf("%s", printf_buf);
    ...
    sprintf(filename, "/usr/local/home/prod/DATA_LOCAL/ESP_datalog/Site%d_chip_id_%x_%d%02d%02d_%02d%02d%02d.log", sitenum+1, CHIP_ID_t, 1900 + current_time->tm_year, 1 + current_time->tm_mon,  current_time->tm_mday, current_time->tm_hour, current_time->tm_min, current_time->tm_sec);
...
*/
    ...
    system("/bin/csh -c \" gzip /usr/local/home/prod/DATA_LOCAL/ESP_datalog/*.log \"");
