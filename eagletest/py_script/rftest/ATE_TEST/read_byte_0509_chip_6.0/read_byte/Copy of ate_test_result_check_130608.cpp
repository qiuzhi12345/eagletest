...
extern double DeepSleep_IDD_VBAT[2];
extern double DeepSleep_IDD_DVDD_IO[2];
...
////////////////////////////////
#ifndef CONVERT_EN
    #define ate_check_print 0    // ATE can modify
#else
...
S8 ate_RX_GAIN_CHECK_LOW_hdb[14] = {15, -27, 15, -5, -7, -5, 8, -2, -2, -2, -2, -2, -2, -2};
S8 ate_RX_GAIN_CHECK_HIGH_hdb[14] = {25, -16, 22, 3, -1, 2, 12, 2, 2, 2, 2, 2, 2, 2};

S8 ate_TX_PWCTRL_ATTEN_LOW_qdb[6] = {-12, -8, 9, 15, 25, 33}; //{-6, -4, 0, 4, 8, 12};
S8 ate_TX_PWCTRL_ATTEN_HIGH_qdb[6] = {20, 26, 35, 41, 51, 59}; //{14, 16, 20, 24, 28, 32};

S8 ate_RX_SWITCH_GAIN_HIGH_db[3] = {4, -6, 7};
S8 ate_RX_SWITCH_GAIN_LOW_db[3] = {-1, -10, 3};

S16 ate_RX_NOISEFLOOR_HIGH[2] = {-396, -380};  //channel 14, 3
S16 ate_RX_NOISEFLOOR_LOW[2] = {-412, -396}; 
...
#define ate_BBRX2_RXIQ_HIGH  6
#define ate_BBRX2_RXIQ_LOW  -6
...
#define ate_TXCAP_PA2G_CCT_STG1_HIGH  9
...
#define ate_TXCAP_TMX2G_CCT_LOAD_check_en 0
#define ate_TXCAP_PA2G_CCT_STG1_check_en 0
#define ate_TXCAP_PA2G_CCT_STG2_check_en 0
...
unsigned char ate_check(unsigned char *data_in,int sitenum)
{
    ...
    U16 addr_offset[] = {1, 5, 6, 8, 9, 11, 14, 23, 32, 41, 50, 92, 98, 104, 110, 113, 116, 119, 125, 139, 140, 141, 146, 151, 156, 161, 221, 281, 
                        341, 401, 405, 406, 407, 409, 412, 416, 417, 427, 437, 442, 447, 452, 455, 459, 467, 468, 470, 471, 486, 501, 502, 508, 509, 510};
...
//#endif
    ...
    U8 noisefloor_fail[2] = {0, 0};
    for(i=0; i<2; i++){
        RX_NOISEFLOOR_t[i] = (RX_NOISEFLOOR[i*2+1] << 8*1) | (RX_NOISEFLOOR[i*2+0] << 8*0);
        if (ate_check_print) PHY_PRINT("RX_NOISEFLOOR, %d\n", RX_NOISEFLOOR_t[i]);
        if((RX_NOISEFLOOR_t[i] < ate_RX_NOISEFLOOR_LOW[i]) || (RX_NOISEFLOOR_t[i] > ate_RX_NOISEFLOOR_HIGH[i]))
            noisefloor_fail[i] = 1;
    }
    if ((noisefloor_fail[0] == 1) && (noisefloor_fail[1] == 1))
        check_ate_data(0, 2, 2, 12);
    ...
    PHY_PRINT("DeepSleep_IDD_VBAT = %f\n",DeepSleep_IDD_VBAT[sitenum]);
    PHY_PRINT("DeepSleep_IDD_DVDD_IO = %f\n",DeepSleep_IDD_DVDD_IO[sitenum]);
...
//   cscExec->Datalog()->printf("%s", printf_buf);
    ...
    sprintf(filename, "../datalog/chip_id_%x_%d%02d%02d_%02d%02d%02d.log", CHIP_ID_t, 1900 + current_time->tm_year, 1 + current_time->tm_mon,  current_time->tm_mday, current_time->tm_hour, current_time->tm_min, current_time->tm_sec);
    //sprintf(filename, "/usr/local/home/test/datalog/124/chip_id_%x_%d%02d%02d_%02d%02d%02d.log", CHIP_ID_t, 1900 + current_time->tm_year, 1 + current_time->tm_mon,  current_time->tm_mday, current_time->tm_hour, current_time->tm_min, current_time->tm_sec);
