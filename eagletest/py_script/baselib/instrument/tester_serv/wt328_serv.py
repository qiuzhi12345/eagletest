...
sys.path.append('D:/chip/lib/wt3xx_ctrl/WTexecution');
sys.path.append('d:/chip/eagletest/py_script/baselib/loglib');
sys.path.append('d:/chip/eagletest/py_script/baselib/test_channel');
from log_lib import *
try:
    import wt3xx_ctrl
    loginfo('Load WT328 Control Module OK!')
except:
    logwarn("import wt328_ctrl fail")
...
def init():
    ...
    try:
        result=wt3xx_ctrl.WT_Initilaze();
        if result==0:
...
def term():
    ...
    loginfo(_connID)
    try:
        result=wt3xx_ctrl.WT_Terminate(_connID);
        if result==0:
...
def open_instru(_ipaddr):
    ...
    result=1;
    try:
        result=wt3xx_ctrl.WT_Connect(_ipaddr);
        _connID=result[1]
        if result[0]==0:
...
def close():
    ...
    loginfo(_connID)
    try:
        result=wt3xx_ctrl.WT_DisConnect(_connID);
        if result==0:
...
def wave_gen_11ax_su(*args):
    result=1
    print args
    try:
        ppduformat,wg_standard,bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,nss,pe,pe_type,spatial_reuse,txop,mac_addr1,save_path=args
        result=wt3xx_ctrl.WT_WaveGenerator_V2_11ax_su(ppduformat,wg_standard,bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,nss,pe,pe_type,spatial_reuse,txop,mac_addr1,save_path);
        print result
        if result==0:
            reply_info='WaveGenerator 11ax_su ok!';
        else:
            reply_info='Fail to WaveGenerator 11ax_su!';
    except:
        reply_info='Exception occur when WaveGenerator 11ax_su!';
    return ('%d,%s'%(result,reply_info));

def wave_gen_11ax_tb(*args):
    result=1
    print args
    try:
        wg_standard,bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,numltfsymbol,nss,pe,pe_type,spatial_reuse,txop,usernum,ruindex,aid,mac_addr1,save_path=args
        result=wt3xx_ctrl.WT_WaveGenerator_V2_11ax_tb(wg_standard,bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,numltfsymbol,nss,pe,pe_type,spatial_reuse,txop,usernum,ruindex,aid,mac_addr1,save_path);
        print result
        if result==0:
            reply_info='WaveGenerator 11ax_tb ok!';
        else:
            reply_info='Fail to WaveGenerator 11ax_tb!';
    except:
        reply_info='Exception occur when WaveGenerator 11ax_tb!';
    return ('%d,%s'%(result,reply_info));

def wave_gen_11ax_mu(*args):
    result=1
    print args
    try:
        wg_standard,bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,nss,pe,pe_type,spatial_reuse,txop,usernum,uservalidmap,sigbmcs,ruallocmap,mac_addr1,save_path,mcs_random_en,psdulen_random_en,uservalidmap_rand_en=args
        result=wt3xx_ctrl.WT_WaveGenerator_V2_11ax_mu(wg_standard,bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,nss,pe,pe_type,spatial_reuse,txop,usernum,uservalidmap,sigbmcs,ruallocmap,mac_addr1,save_path,mcs_random_en,psdulen_random_en,uservalidmap_rand_en);

##        result=wt3xx_ctrl.WT_WaveGenerator_V2_11ax_mu(args)
##        if result==0:
##            reply_info='WaveGenerator 11ax_mu ok!';
##        else:
##            reply_info='Fail to WaveGenerator 11ax_mu!';
    except:
        reply_info='Exception occur when WaveGenerator 11ax_mu!';
##    return ('%d,%s'%(result,reply_info));
    return result
...
def readcmd(que):
    try:
        ...
        elif cmd_lst[0]=='wave_gen_11ax_su':
            reply=wave_gen_11ax_su(cmd_lst);
...
##'WT_DEMOD_UNKNOW')
##WT_DEMOD_ENUM_dict=dict(zip(range(30),WT_DEMOD_ENUM))




def wg_11ax_su_er_param():
    ppduformat,wg_standard,bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,nss,pe,pe_type,spatial_reuse,txop,mac_addr1=1,22,20,3,512,24,7,0,0,3,1,0,0,0,127,"aabbccddeeff"

    ppduformat_list={1,0}
    stbc_list={0,1}
    ##mcs_index_list1=range(0,10)
    mcs_index_list1={7,9}
    mcs_index_list2={0,1,2}
    mcs_index_list3={7,9}
    giltfsize_list={2}
    pe_list={0}
    petype_list={0}
    psdulen_list={4096}
    ##psdulen_list=range(40,1025,164)
    psdutype_list={3}

    for ppduformat in ppduformat_list:
        if ppduformat==1:
            _save_path="d:/chip/lib/wt3xx_ctrl/wave/11ax/ax_su_"
            mcs_stop=mcs_index_list1
        else:
            _save_path="d:/chip/lib/wt3xx_ctrl/wave/11ax/ax_er_"
            mcs_stop=mcs_index_list2
        for stbc in stbc_list:
            for mcs_index in mcs_stop:
                for giltfsize in giltfsize_list:
                    for pe in pe_list:
                        for pe_type in petype_list:
                            for psdutype in psdutype_list:
                                psdulen_list=range(40*(1+mcs_index),500*(1+mcs_index),70*(1+mcs_index))
                                for psdulen in psdulen_list:
                                    scrambler_cnt=24
                                    bsscolor=0
                                    spatial_reuse=0
                                    txop=127
...
##                                    scrambler_cnt=random.randint(1,120)
##                                    bsscolor=random.randint(1,63)
##                                    spatial_reuse=random.randint(1,15)
##                                    txop=random.randint(0,126)


                                    save_path=_save_path+"bw%d_psdutype%d_psdulen%d_scrambler%d_mcs%d_stbc%d_bsscolor%d_giltfsize%d_nss%d_pe%d_petype%d_spatialreuse%d_txop%d_macaddr1%s.bwv"%(bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,nss,pe,pe_type,spatial_reuse,txop,mac_addr1)

                                    res= wave_gen_11ax_su(ppduformat,wg_standard,bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,nss,pe,pe_type,spatial_reuse,txop,mac_addr1,save_path)
                                    print res
                                    time.sleep(0.1)

def wg_11ax_tb_param():
    wg_standard,bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,numltfsymbol,nss,pe,pe_type,spatial_reuse,txop,usernum,ruindex,aid,mac_addr1=22,20,3,512,24,7,0,0,3,0,1,0,0,0,127,1,39,1,"aabbccddeeff"
    ...
    numltfsymbol_list={0}
    ruindex_list1={54}
    ruindex_list2={12,40,55}
    bw_list={20}

    _save_path="d:/chip/lib/wt3xx_ctrl/wave/11ax_tb/ax_tb_"
    for bw in bw_list:
        if bw==20:
            ruindex_list=ruindex_list1
        else:
            ruindex_list=ruindex_list2
        for numltfsymbol in numltfsymbol_list:
            for mcs_index in mcs_index_list3:
                for giltfsize in giltfsize_list:
                    for ruindex in ruindex_list:
                        for psdutype in psdutype_list:
##                            psdulen_list=range(40*(1+mcs_index),500*(1+mcs_index),70*(1+mcs_index))
                            for psdulen in psdulen_list:
##                                scrambler_cnt=24
...
##                                txop=127
                                ...
                                save_path=_save_path+"bw%d_psdutype%d_psdulen%d_scrambler%d_mcs%d_stbc%d_bsscolor%d_giltfsize%d_numltfsymbol%d_nss%d_pe%d_petype%d_spatialreuse%d_txop%d_usernum%d_ruindex%d_aid%d_macaddr1%s.bwv"%(bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,numltfsymbol,nss,pe,pe_type,spatial_reuse,txop,usernum,ruindex,aid,mac_addr1)

                                res= wave_gen_11ax_tb(wg_standard,bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,numltfsymbol,nss,pe,pe_type,spatial_reuse,txop,usernum,ruindex,aid,mac_addr1,save_path)
                                print res
                                time.sleep(0.1)

def wg_11ax_mu_param():
    wg_standard,bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,nss,pe,pe_type,spatial_reuse,txop,aid,usernum,uservalidmap,sigbmcs,ruallocmap,mac_addr1,mcs_random_en,psdulen_random_en=22,20,3,512,24,7,0,0,3,1,0,0,0,127,0,4,1,0,0,"aabbccddeeff",1,1

    ruallocmap_list=range(0,17)+range(24,97,8)+[112,128,192]
    usernum_list=[9,8,8,7,8,7,7,6,8,7,7,6,7,6,6,5,3,3,6,5,5,4,6,5,5,4,2,4,3,1]
    usernum_dic=dict(zip(ruallocmap_list,usernum_list))

    ruallocmap_list1=[56]
    mcs_index_list3=[0]
##    giltfsize_list=[random.randint(1,3)]
    pe_list={0}
    petype_list={0}
    psdulen_list={128}
    ##psdulen_list=range(40,1025,164)
    psdutype_list={1,2}
    bw_list={20}
##    sigbmcs_list=[random.randint(0,5)]
    mcs_random_en,psdulen_random_en,uservalidmap_rand_en=0,0,0

    _save_path="d:/chip/lib/wt3xx_ctrl/wave/11ax_mu/ax_mu_"
    for bw in bw_list:
        for ruallocmap in ruallocmap_list1:
            usernum=usernum_dic[ruallocmap]
            sigbmcs_list=range(1,6)
            giltfsize_list=[3]
            for sigbmcs in sigbmcs_list:
                for giltfsize in giltfsize_list:
                    for psdutype in psdutype_list:
                        if mcs_random_en==0:
                            for mcs_index in mcs_index_list3:
...
##                                    txop=random.randint(0,126)
                                    ...
                                    save_path=_save_path+"bw%d_psdutype%d_psdulen%d_giltfsize%d_usernum%d_uservalidmap%d_sigbmcs%d_ruallocmap%d_scrambler%d_stbc%d_bsscolor%d_nss%d_pe%d_petype%d_spatialreuse%d_txop%d.bwv"%(bw,psdutype,psdulen,giltfsize,usernum,uservalidmap,sigbmcs,ruallocmap,scrambler_cnt,stbc,bsscolor,nss,pe,pe_type,spatial_reuse,txop)

                                    res= wave_gen_11ax_mu(wg_standard,bw,psdutype,psdulen,scrambler_cnt,mcs_index,stbc,bsscolor,giltfsize,nss,pe,pe_type,spatial_reuse,txop,usernum,uservalidmap,sigbmcs,ruallocmap,mac_addr1,save_path,mcs_random_en,psdulen_random_en,uservalidmap_rand_en)
                                    print res
                                    if res[0]==0:
                                        os.rename(save_path,_save_path+"bw%d_mcs%s_psdutype%d_psdulen%s_giltfsize%d_usernum%d_uservalidmap%s_aid%s_sigbmcs%d_ruallocmap%d_scrambler%d_stbc%d_bsscolor%d_nss%d_pe%d_petype%d_spatialreuse%d_txop%d.bwv"%(bw,res[1],psdutype,res[2],giltfsize,usernum,res[4],res[3],sigbmcs,ruallocmap,scrambler_cnt,stbc,bsscolor,nss,pe,pe_type,spatial_reuse,txop))
...
##                                txop=127
                                ...
                                print res
                                if res[0]==0:
                                    os.rename(save_path,_save_path+"bw%d_mcs%s_psdutype%d_psdulen%s_giltfsize%d_usernum%d_uservalidmap%s_aid%s_sigbmcs%d_ruallocmap%d_scrambler%d_stbc%d_bsscolor%d_spatialreuse%d_txop%d.bwv"%(bw,res[1],psdutype,res[2],giltfsize,usernum,res[4],res[3],sigbmcs,ruallocmap,scrambler_cnt,stbc,bsscolor,spatial_reuse,txop))
...
if __name__ == '__main__':
    res=init();
    print res
    res=open_instru("192.168.10.254")
    print res
    wg_11ax_mu_param()
