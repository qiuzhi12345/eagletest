...
def check_supported_function(func, check_func):
    ...
    functionality between the ESP8266 & ESP32/32C ROM loaders, and the
...
def stub_and_esp32_function_only(func):
    """ Attribute for a function only supported by software stubs or ESP32/32C ROM """
...
class ESPLoader(object):
    ...
    # Commands supported by ESP32C ROM bootloader only
    ...
    def detect_chip(port=DEFAULT_PORT, baud=ESP_ROM_BAUD, connect_mode='default_reset', trace_enabled=False):
        ...
        for cls in [ESP8266ROM, ESP32ROM, ESP32CROM]:
    ...
    def flash_begin(self, size, offset, encrypted_write=False):
        ...
        args = struct.pack('<IIII', erase_size, num_blocks, self.FLASH_WRITE_SIZE, offset)
        if isinstance(self, ESP32CROM):
            args += struct.pack('<I', 1 if encrypted_write else 0)
        elif encrypted_write:
            raise FatalError("Encrypted write currently supported on ESP32C ROM only")

        self.check_command("enter Flash download mode", self.ESP_FLASH_BEGIN, args, timeout=timeout)
    ...
    def flash_defl_begin(self, size, compsize, offset):
        ...
        args = struct.pack('<IIII', write_size, num_blocks, self.FLASH_WRITE_SIZE, offset)
        if isinstance(self, ESP32CROM):
            args += struct.pack('<I', 0)  # encrypted_write byte, not supported for compressed flashing

        self.check_command("enter compressed flash mode", self.ESP_FLASH_DEFL_BEGIN, args, timeout=timeout)
        if size != 0 and not self.IS_STUB:
    ...
    def run_spiflash_command(self, spiflash_command, data=b"", read_bits=0):
        ...
        # following two registers are ESP32 & 32C only
        if self.SPI_MOSI_DLEN_OFFS is not None:
            # ESP32/32C has a more sophisticated wayto set up "user" commands
...
class ESP32CROM(ESP32ROM):
    CHIP_NAME = "ESP32C"

    IROM_MAP_START = 0x40080000
    IROM_MAP_END   = 0x40c00000
    DROM_MAP_START = 0x3F000000
    DROM_MAP_END   = 0x3F400000

    DATE_REG_VALUE = 0x00000500  # This is actually UART_ID_REG(0) on ESP32C
...
class ESP32CFirmwareImage(ESP32FirmwareImage):
    """ ESP32C Firmware Image almost exactly the same as ESP32FirmwareImage """
    ROM_LOADER = ESP32CROM
...
def detect_flash_size(esp, args):
    if args.flash_size == 'detect':
        try:
            ...
            raise FatalError("'--flash-size detect' cannot be used in secure download mode")
    ...
    for address, argfile in args.addr_filename:
        ...
        if args.compress:
            ...
            try:
                ...
                raise FatalError("Compressed uploads are not supported in secure download mode")
        else:
            ratio = 1.0
            blocks = esp.flash_begin(uncsize, address, args.encrypt)
        ...
        try:
            all_erased = hashlib.md5(b'\xFF' * uncsize).hexdigest()
            res = esp.flash_md5sum(address, uncsize)
            if not args.encrypt:
                if res != calcmd5:
                    print('File  md5: %s' % calcmd5)
                    print('Flash md5: %s' % res)
                    print('MD5 of 0xFF is %s' % (all_erased))
                    raise FatalError("MD5 of file does not match data in flash!")
                else:
                    print('Hash of data verified.')
            else:
                # For encrypted writes, we can't verify easily. However we can verify that it's not all erased bytes
                # (which will happen if SPI flash encryption is not correctly configured)
                if res == all_erased:
                    raise FatalError("MD5 of flash suggests that writing failed. Is flash encryption correctly configured in hardware?")
...
def elf2image(args):
    ...
    elif args.chip == 'esp32c':
        image = ESP32CFirmwareImage()
...
def main():
    ...
    parser.add_argument('--chip', '-c',
                        ...
                        choices=['auto', 'esp8266', 'esp32', 'esp32c'],
    ...
    parser_write_flash.add_argument('--encrypt', '-e', help='Apply flash encryption when writing data (requires correct efuse settings)', action='store_true') 
...
S0yomEyqQut//QdAk8Ky\
""")))
ESP32CROM.STUB_CODE = eval(zlib.decompress(base64.b64decode(b"""
eNqNWvt31MYV/ldk4SfgViNptSPOSbMGuhg4bQIJxqF7ktXThhxc4uyJTUL+9853H5rR7rrtD7ZHo9GdO/fx3cf4j4NVd7s6eBTVB4vb3kbuV/41RgmNipPFbeKGpXGPrfvpF7dNEvGktYuV+41Rsnt2ym9pZfX/\
rDSgl/AC/TGJcpAEo+DHKkfdxL1umJLFnjWPEzeXpMPeR/gm5Gpng734PVOUx8PF6ux6k38iA+Im1ZO45zw6TLafJElmzGrn+TSF46TyPHdtILNmbc+y5D39BDFx9vlu6Q0/xo9tEnwNpbZfMwH9Sfgcgbp3lJs0\
xivw/cQNJjiJ9SfpKn5bTVT6549ZRL2KKjsBWbx669Zhtj6PwdMbqNqdrZlgRSpEoboM0o5n5+7R7Ln5LFBxImMcawIKr/yk1xXkNuEv2nT08vHlSNOnJM+VELWPT2NRdZM8ykHocRmvCVqFCLskdpM1I6WHMglk\
jW/Im5qvA8NR7eTh82ymo1MwId+YEel8IO2VRRKGgKYYnOhgOOg5XOLFE1ieGb04dL+KKFqJQlN4aVPcwyu3uOt4cW38eBBzle7LgH5WTldVIabm/jZiHDDtLjT5OqBZB/os+W+NHxkPezXifqVTp6lH27KwGa34\
JQwdc6XMef9J/yVCd/zYZnSWU3kTbECcm4BDoVdOscffws8tn5XOl3iXbsm9p+zzppwzMCXJF7cMRuHeGIjZvVmNHVhIbyAAJvKBzt1gUXqGWjPg4kqOno2Eci7b1vG+EAZ6OhW2pagc41zBwtlbM7aFH/iTdUCC\
JKomAEL3qyIy53kyacTkMrg8nD1/9923i4VbYwul0bGc2Dufsn0ZlbXdZQkS+qQcD1T4Ic5COSaHrLLIsVunkYCJQEMXuLVtHsVsWU1+9D1x9ejND/gDlnFguJ8/6CfyGRdPyM3H8UpcGUBa9jIBxKgkNlQ+jLGg\
wZNIHB5fmwgCzJTdap3dfyyufEDq3N+6EpQ3bMhV6iMA3MsoOAJX2y4IPWmAsem6n/vQXUVjmHXztexXtQLdcIF+U7JVssuGt24lZFQahRIN7pW5CdiTKElnSvR8L7Ay/YBH0txJjHGGr2t4aYU4U96/z9taXuPT\
A1u+OHo3BLTjxZUQbyb46xCwXGMWgMHgO1MulSEgNQkgZZMETMF4K3nf1uMUZSQYXdOI5Wdj2vSt0rRCZ/pf6LSyJt9cs5ky8EkegXbhUUXp6bOpY8m3agW0VPB2awqn4/PwQdwWZp/OOPBVHYlzN5iu/JpiZ4fZ\
AIKRjre6AYltgE8kBIVEGGjEfPsEQN9KpqJKMn5ZSAlehB10PZPYwkW2/u2LeEaoetawvVZGqIGR4GvgfVWJn3Rb1Ij5MkiHav1mz9sqGQmLnuCqWVfE4EaBgWLHpv1fRnEZquxT+LAKH27Dh8NW4I1cZEIPeSue\
