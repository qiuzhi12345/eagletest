...
#define TEXT_ADDRESS 0x40070000
#define TEXT_END     0x400E0000
#define DATA_ADDRESS 0x3ff80000
#define DATA_END 0x3ffe0000
...
struct header {
    ...
    unsigned char spi_mode;  //pad 2 bytes, since iram can't be access in byte-wise
    unsigned char spi_clk;  //pad 2 bytes, since iram can't be access in byte-wise
    unsigned int entry;
	unsigned char reserves[16];
};
...
int main(int argc, char const *  argv[]) 
{
    ...
    ofstream binfile;
    int pad, blocks = 0;
    unsigned int text_entry_address, data_start_address, rodata_start_address, text_load_address;
    ...
    if (1) {
        sscanf(argv[7], "%08x", &text_load_address);
        cout << "text load_addr 0x" << hex << text_load_address << endl;

        sscanf(argv[2], "%08x", &text_entry_address);
        cout << "text entry_addr 0x" << hex << text_entry_address << endl;
	    ...
	    text_header.load_address = text_load_address;
        } else {
    ...
    if (text_size) {
        ...
        if (binfile.is_open()) {
            ...
            bin_header.spi_mode = SPI_MODE;
            bin_header.spi_clk = SPI_CLK;
